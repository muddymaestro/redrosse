...
export default {
	...
	actions: {
		...
		async sendPost({ commit }, {id, formData}) {
			const response = await axios.post(`wall-post/${id}`, formData)
		...
		async fetchPosts({ commit }) {
			const response = await axios.get('wall-posts')
			commit('FETCH_POSTS', response.data.posts)
		},

		// liking the post
		async likePost({ commit }, postId) {
			const response = await axios.get(`like-wall-post/${postId}`)
		...
		async unlikePost({ commit }, postId) {
			const response = await axios.get(`unlike-wall-post/${postId}`)
		...
		async commentPost({ commit }, {post_id, formData}) {
			const response = await axios.post(`wall-post-comment/${post_id}`, formData)
